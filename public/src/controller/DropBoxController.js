class DropBoxController{
    constructor(){
        ...
        this.selectonchange = new Event('onselectonchange');
        this.btnSendFile = document.querySelector('#btn-send-file');
        this.inputEl = document.querySelector('#files');
        this.snackBar = document.querySelector('#react-snackbar-root');
        this.snackBarProgress = document.querySelector('.mc-progress-bar-fg');
        this.snackFileName = this.snackBar.querySelector('.filename');
        this.snackTimeleft = this.snackBar.querySelector('.timeleft');
        this.listElements = document.querySelector('.list-unstyled');
    ...
    getLengthLi(){
        return this.listElements.querySelectorAll('li.selected');
    };
    ...
    initEvents(){
        ...
        this.inputEl.addEventListener('change', e => {
            ...
            this.uploadTask(e.target.files).then(response => {
                response.forEach(resp => {
                    return this.getFirebaseRef().push().set(resp.files['input-file']);
                });
                this.uploadEnd();
            }).catch(e => {
                this.uploadEnd();
        ...
        this.btnDelete.addEventListener('click', e => {
            this.removeTask().then(response => {
                response.forEach(resp => {
                    if(resp.fields.key){
                        return this.getFirebaseRef().child(resp.fields.key).remove();
                    }
                });
            }).catch(e => {
                console.log(e);
        ...
        this.listElements.addEventListener('onselectonchange', e => {
            switch(this.getLengthLi().length){
                case 0:
        ...
        this.btnNewFolder.addEventListener('click', e => {
            let name = prompt('Criar nova pasta:');
            if(name){
    ...
    removeTask(){
        ...
        this.getLengthLi().forEach(li => {
            let key = li.dataset.key;
            let file = JSON.parse(li.dataset.file);
            let formData = new FormData();
            formData.append('key', key);
            formData.append('path', file.path);
            promises.push(this.ajax('/file', 
                                    'DELETE', 
                                    formData));
        });
    ...
    ajax(url, method = 'GET', formData = new FormData(), uploadTime = function(){}, progress = function(){}){
        return new Promise((resolve, reject) => {
            let ajax = new XMLHttpRequest();
                ...
                ajax.upload.onprogress = uploadTime;
    ...
    uploadTask(files){
        ...
        [...files].forEach(file => {
            ...
            promises.push(this.ajax('/upload', 
                                    'POST', 
                                    formData, 
                                    () => {
    ...
    uploadProgress(event, file){
        let timestemp = Date.now() - this.startUpload;
        let loaded = event.loaded;
        let total = event.total;
        let porcent = parseInt((loaded / total) * 100);
        let timeleft = ((100 - porcent) * timestemp) / porcent;
        this.snackBarProgress.style.width = `${porcent}%`;
        this.snackFileName.innerText = this.longName(file);
        this.snackTimeleft.innerText = this.durationTime(timeleft);

    };
    longName(file){
        if(file.name.length > 6){
            let name = file.name.substring(0, 6);
            return `${name}...`;
        };
    };
    ...
    initEventsLi(li){
        ...
        li.addEventListener('click', e => {
            if(e.shiftKey){
                let firstLi = this.listElements.querySelector('li.selected');
                if(firstLi){
                    let indexStart;
                    let indexEnd;
                    let lis = firstLi.parentElement.childNodes;
                    ...
                    this.listElements.dispatchEvent(this.selectonchange)
                    return true;
                };   
            };
            if(!e.ctrlKey){
                this.listElements.querySelectorAll('li.selected').forEach(el => {
    ...
    readFiles(){
        ...
        this.getFirebaseRef().on('value', snapshot => {
            ...
            snapshot.forEach(snap => {
                ...
                let key = snap.key;
                if(data.type){
                    this.listElements.appendChild(this.getFileli(data, key));
                };
            });
        });
    };
